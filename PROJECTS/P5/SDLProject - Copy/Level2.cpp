...
#define LEVEL2_HEIGHT 92
#define ENEMY_COUNT_2 1
#define PUP_COUNT_2 3
...
void Level2::Initialize() {
	...
	state.pup[0].position = glm::vec3(3, -80, 0);
	state.pup[1].position = glm::vec3(9, -51, 0);
	state.pup[2].position = glm::vec3(4, -33, 0);

}
void Level2::Update(float deltaTime) {
	state.player->Update(deltaTime, state.player, state.enemies, ENEMY_COUNT_2, state.pup, PUP_COUNT_2, state.map);
	if (state.player->position.y >= -1.9) {
		state.nextScene = 3;
	}
	for (int i = 0; i < ENEMY_COUNT_2; i++) {
		state.enemies[i].Update(deltaTime, state.player, state.enemies, ENEMY_COUNT_2, state.pup, PUP_COUNT_2, state.map);

	}
	for (int i = 0; i < PUP_COUNT_2; i++) {
		state.pup[i].Update(deltaTime, state.player, state.enemies, ENEMY_COUNT_2, state.pup, PUP_COUNT_2, state.map);
